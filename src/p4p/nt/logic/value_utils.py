...
def recurse_values(
    value1: Value,
    value2: Value,
    func: Callable[[Value, Value, str], None],
    keys: Optional[List[str]] = None,
) -> bool:
    """Recurse through two Values with the same structure and apply a supplied function to the leaf nodes"""
    if not keys:
...
def overwrite_marked(
    current: Value, update: Value, fields: Optional[List[str]] = None
) -> None:
    """
    Overwrite all of the marked fields in one Value with fields from another Value.

    This makes the changes in place rather than returning a copy.
    """

    def overwrite_changed_key(
        update_leaf: Value, current_leaf: Value, key: str
    ) -> None:
        """
